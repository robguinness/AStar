...
function [pathMatrix, pathArray, Gcost] = AStar(startX, startY, targetX, targetY, latitude, longitude, inverseSpeed, whichList, waypoints, delayOption, drawOption, smoothingOn)
    ...
    numNeighbors = 8;   % The number of neighbors to use
    tmpNewOpenListItemID = zeros(numNeighbors,'uint32');
    
    iceBreakerDistThreshold = 1;
    ...
    mapWidth = size(whichList, 2);
    mapHeight = size(whichList, 1);

    parentX = sparse(mapHeight,mapWidth);        % 2D array stores x coord of parent for every cell
    parentY = sparse(mapHeight,mapWidth);        % 2D array stores y coord of parent for every cell
    Gcost = zeros(mapHeight,mapWidth);          % 2D array stores G cost of each cell
    ...
    while (1)
        ...
        if (numberOfOpenListItems ~= 0)
            ...
            % temporary variable to assign to whichList after parfor loop
            tmpWhichList = struct('a',{},'b',{});
            
            % temporary variable to assign to openList after parfor loop
            tmpOpenList = struct('id',{},'x',{}, 'y',{});
            
            
            
            %tmpNewOpenListItemID = zeros(8,'uint32');
            for j=1:numNeighbors
                tmpNewOpenListItemID(j) = newOpenListItemID + j;
            end
            newOpenListItemID = newOpenListItemID + numNeighbors;
            
            %test = 100;
            tmpNumberOfNewOpenListItems = 0;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            parfor j=1:numNeighbors %% originally j=56, see calculateGCost3 at the bottom
                            ...
                            %if (whichList(a,b) ~= unwalkable)
                                ...
                                if (corner == walkable)
                                    ...
                                    if (whichList(a,b) ~= onOpenList)
                                        ...
                                        tmpOpenList(j).id = tmpNewOpenListItemID(j);
                                        %openList(m) = tmpNewOpenListItemID(j);
                                        
                                        tmpOpenList(j).x = a;
                                        tmpOpenList(j).y = b;
                                        %openX(tmpNewOpenListItemID(j)) = a;
                                        %openY(tmpNewOpenListItemID(j)) = b;
...
%                                         end
                                        ...
                                        Hcost(tmpOpenList(j).id) = 10*sqrt((a - targetX)^2 + (b - targetY)^2) / maxSpeed; %Euclidean distance
                                        Fcost(tmpOpenList(j).id) = Gcost(a,b) + Hcost(openList(m));
            ...
            for j=1:numNeighbors
                whichList(tmpWhichList(j).a,tmpWhichList(j).b) = onOpenList;
            end    
...
% The line below can be ommited if MyGrid is used - I think so
